...
class Roi(object):
    ...
    _shape_list=['Polygon', 'Rectangle', 'Ellipse' , 'Line', 'Point', 'Polyline']
    
    
    def __init__(self, roi_dict):
                
        self.dictionary=roi_dict
    ...
    def roi_to_coordinates(self):
        ...
        if 'Shape' in self.dictionary['Union'].keys():
            if isinstance(self.dictionary['Union']['Shape'], dict):
                shape_list=[self.dictionary['Union']['Shape']]
            elif isinstance(self.dictionary['Union']['Shape'], list):
                shape_list=self.dictionary['Union']['Shape']
        else:
            if isinstance(self.dictionary['Union'], dict):
                shape_list=[self.dictionary['Union']]
            elif isinstance(self.dictionary['Union'], list):
                shape_list=self.dictionary['Union']
