    ...
    if 'ROI' in metadata.keys():
        ...
        for index, roi in enumerate(metadata['ROI']):
            ...
            for shape in shape_list:
                ...
                shape_attrib={str(key):str(shape[key]) for key in shape.keys() if key not in RoiClass._shape_list}
                shape_element=etree.Element('Shape', shape_attrib)
            
                #Create shape_type element. Shape can only contain one shape type
                shape_type_key=[key for key in shape.keys() if key in RoiClass._shape_list][0]
...
def save_image(image, metadata, path):
    ...
    if metadata["DimensionOrder"] in __dim_orders:
